...
def inference(images):
    ...
    w_fc3 = tf.get_variable('w_fc3', [512,512], trainable=FLAGS.trainable)
    b_fc3 = tf.get_variable('b_fc3', initializer=tf.constant(0.1, shape=[512]), trainable=FLAGS.trainable)
    h_fc3 = tf.nn.relu(tf.matmul(h_fc2_dropout, w_fc3)+b_fc3)
    h_fc3_dropout = tf.nn.dropout(h_fc3, keep_prob)

    w_output = tf.get_variable('w_output', [512, 18], trainable=FLAGS.trainable)
    b_output = tf.get_variable('b_output', initializer=tf.constant(0.1, shape=[5]), trainable=FLAGS.trainable)
    output = tf.matmul(h_fc3_dropout, w_output)+b_output
