...
def grasps_to_bboxes(bboxes):
    # converting and scaling bounding boxes into grasps, g = {x, y, tan, h, w}
    labels = tf.unstack(bboxes, axis=1)
    x = labels[0]
    y = labels[1]
    x_vec = labels[2] * 100
    y_vec = labels[3] * 100
    h = labels[4]
    return x, y, x_vec, y_vec, h
...
def run_training():
    ...
    with g.as_default():
        ...
        x, y, x_vec, y_vec, h = grasps_to_bboxes(bboxes) # x, y, tan, h, w = bboxes_to_grasps(bboxes)


        x_hat, y_hat, x_vec_hat, y_vec_hat, h_hat = tf.unstack(inference(images), axis=1) # list

        gamma = tf.constant(1.0)
        loss = tf.reduce_mean(tf.pow(x_hat -x, 2) +tf.pow(y_hat -y, 2) + gamma*tf.pow(x_vec_hat - x_vec, 2) +gamma*tf.pow(y_vec_hat - y_vec, 2) + tf.pow(h_hat - h, 2))
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                #train  
                if FLAGS.train_or_validation == 'train':
                    _, loss_value, images_np, x_value, x_model, y_value, y_model, x_vec_value, x_vec_model, y_vec_value, y_vec_model, h_value, h_model = sess.run([train_op, loss, images, x, x_hat, y, y_hat, x_vec, x_vec_hat, y_vec, y_vec_hat, h, h_hat])
                    ...
                    if step % 100 == 0:
                        print('Step %d | loss = %s\n | x     = %s\n | x_hat = %s\n | y     = %s\n | y_hat = %s\n | x_vec     = %s\n | x_vec_hat = %s\n | y_vec     = %s\n | y_vec_hat = %s\n | h =     %s\n | h_hat = %s\n | x_compatibility = %s\n | y_compatibility = %s\n | %.3f sec/batch\n' \
                            %(step, loss_value, x_value[:3], x_model[:3], y_value[:3], y_model[:3], x_vec_value[:3], x_vec_model[:3], y_vec_value[:3], y_vec_model[:3], h_value[:3], h_model[:3], (x_model[:3]-x_value[:3])/10, (y_model[:3]-y_value[:3])/10, duration)
