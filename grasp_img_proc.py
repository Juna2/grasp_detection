...
def parse_example_proto(examples_serialized):
    ...
    r = 5*tf.random_uniform((1,), minval=0, maxval=tf.size(bboxes, out_type=tf.int32)/5, dtype=tf.int32) # r = 8*tf.random_uniform((1,), minval=0, maxval=tf.size(bboxes, out_type=tf.int32)/8, dtype=tf.int32) 
    bbox = tf.gather_nd(bboxes, [r,r+1,r+2,r+3,r+4]) # bbox = tf.gather_nd(bboxes, [r,r+1,r+2,r+3,r+4,r+5,r+6,r+7])
